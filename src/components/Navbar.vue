            ...
            <div class="flex space-x-4 items-center h-full">
              <template v-for="item in navigation" :key="item.name">
                <Menu v-if="item.children" as="div" class="relative">
                  <MenuButton
                    :class="[item.current ? 'bg-gray-900 text-white' : 'text-white hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium flex items-center gap-1']">
                    {{ item.name }}
                  ...
                  <transition enter-active-class="transition ease-out duration-100"
                    ...
                    <MenuItems
                      ...
                      <Menu v-if="child.children" as="div" class="relative">
                        <MenuButton
                          :class="[active ? 'bg-gray-100' : '', 'px-4 py-2 text-sm text-gray-700 flex items-center gap-1']">
                          {{ child.name }}
                          <ChevronDownIcon class="size-4" />
                        </MenuButton>
                        <transition enter-active-class="transition ease-out duration-100"
                          ...
                          <MenuItems
                            ...
                            <MenuItem v-for="subChild in child.children" :key="subChild.name" v-slot="{ active }">
                            <RouterLink :to="subChild.href"
                      ...
                      <RouterLink v-else :to="child.href"
                        :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']">
                        {{ child.name }}
                      </RouterLink>
                      </MenuItem>
      ...
      <div class="space-y-1 px-2 pb-3 pt-2">
        <template v-for="item in navigation" :key="item.name">
          <Disclosure v-if="item.children" v-slot="{ open: childOpen }">
            <DisclosureButton
              :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'flex w-full justify-between rounded-md px-3 py-2 text-base font-medium']">
              {{ item.name }}
            ...
            <DisclosurePanel class="ml-4 space-y-1">
              <template v-for="child in item.children" :key="child.name">
                <Disclosure v-if="child.children" v-slot="{ open: subOpen }">
                  <DisclosureButton
                    class="flex w-full justify-between rounded-md px-3 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white">
                    {{ child.name }}
                    <ChevronDownIcon :class="[subOpen ? 'rotate-180' : '', 'size-4 transform transition']" />
                  </DisclosureButton>
                  <DisclosurePanel class="ml-4">
                    <RouterLink v-for="subChild in child.children" :key="subChild.name" :to="subChild.href"
...
const navigation = [
  ...
  {
    name: 'product', href: '/product', current: false, children: [
      {
        name: 'XGS-Pon/G-Pon', href: '/product', children: [
      ...
      {
        name: 'Switch', href: '/product', children: [
